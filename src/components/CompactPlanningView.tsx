...
const CompactPlanningView: React.FC<CompactPlanningViewProps> = ({ planning }) => {
  ...
  const [sortBy, setSortBy] = useState<'time' | 'deliveryNote'>('time');
    ...
    return planning.jobs
      ...
      .filter(j => {
        ...
        return (
          ...
          (j.customName || j.type).toLowerCase().includes(searchLower) ||
          (j.deliveryNote && j.deliveryNote.toLowerCase().includes(searchLower))
        );
      })
      .sort((a, b) => {
        if (sortBy === 'time') {
          ...
          // Ordenar por número de albarán
          const aNote = a.deliveryNote || '';
          const bNote = b.deliveryNote || '';
          return aNote.localeCompare(bNote);
        }
      });
  }, [planning.jobs, startDate, endDate, searchTerm, planning.clients, sortBy]);
    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden relative">
        ...
        <div className="bg-white border-b border-slate-200 px-4 py-3 flex justify-between items-center shrink-0 shadow-sm z-30">
            ...
            <div className="flex items-center gap-3">
               ...
               <button 
                  onClick={() => setSortBy(sortBy === 'time' ? 'deliveryNote' : 'time')} 
                  className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors text-[10px] font-black uppercase tracking-widest"
                  title={`Ordenar por ${sortBy === 'time' ? 'número de albarán' : 'hora de inicio'}`}
               >
                  <ArrowUpDown className="w-3.5 h-3.5" /> 
                  {sortBy === 'time' ? 'Albarán' : 'Hora'}
               </button>
