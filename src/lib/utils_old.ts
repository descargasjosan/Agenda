...
import { getAllHolidays } from './constants';
...
export const getAllHolidays = (year: number): Holiday[] => {
  ...
  // Festivos dinámicos del año actual
  const easterDate = getEasterDate(year);
  const easterMonday = new Date(easterDate);
  easterMonday.setDate(easterMonday.getDate() + 1);
  holidays.push({ date: easterMonday.toISOString().split('T')[0], name: 'Lunes de Pascua' });

  const immaculateConceptionDate = new Date(year, 11, 8);
  holidays.push({ date: immaculateConceptionDate.toISOString().split('T')[0], name: 'Inmaculada Concepción' });

  const constitutionDate = new Date(year, 11, 6);
  holidays.push({ date: constitutionDate.toISOString().split('T')[0], name: 'Día de la Constitución' });
...
export const isHoliday = (dateStr: string, customHolidays: Holiday[] = []): Holiday | undefined => {
  ...
  // Usar festivos dinámicos del año actual
  const currentYear = new Date(dateStr).getFullYear();
  const staticHoliday = getAllHolidays(currentYear).find(h => h.date === dateStr);
