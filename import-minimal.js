// Importación mínima - solo nombre y logo
import fs from 'fs';
...
async function importMinimal() {
  try {
    ...
    if (backupData.clients && backupData.clients.length > 0) {
      ...
      try {
        // Solo campos básicos que seguro existen
        const minimalClients = backupData.clients.map(client => ({
          id: client.id,
          name: client.name || '',
          logo: client.logo || '?'
        }));
        
        console.log(`🧹 Clients mínimos: ${minimalClients.length}`);
        
        const clientsResponse = await fetch(`${NEW_SUPABASE_URL}/rest/v1/clients`, {
          method: 'POST',
          headers,
          body: JSON.stringify(minimalClients)
        });
        
        if (clientsResponse.ok) {
          results.push(`✅ ${backupData.clients.length} clients importados (mínimo)`);
        } else {
